...
#define constraint_float(x, a, b) {min(b, max(x, a))}

// Transform from axis coordinate to servo coordinate
#define gearbox_transform(x, bias, ratio) {ratio * x + bias}

// gear ratio can not be 0
#define inverse_gearbox_transform(x, bias, ratio) {(x - bias) / ratio}
...
class Actuators{
    ...
    // Set commands
    void set_angles(float eta, float xi) {_eta = eta; _xi = xi;};
    void set_thrusts(float w1, float w2) {_w1 = w1; _w2 = w2;};
    ...
    float _servo_limit(float angle, const float gear_ratio, const float min_value, const float max_value);
};
