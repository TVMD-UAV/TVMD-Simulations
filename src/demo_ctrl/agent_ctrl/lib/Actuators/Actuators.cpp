...
float Actuators::_servo_model(struct ServoState & s, float command){
    // The following discrete state space model is obtained with sampling time = 0.01
    float n_state_x =  0.9907 * s.x + 0.009033 * s.dx + 0.00935 * command;
    float n_state_v = -1.807  * s.x + 0.81     * s.dx + 1.807   * command;
    s.x = n_state_x;
    s.dx = n_state_v;
    return s.x;
}

float Actuators::_servo_limit(float angle, const float gear_ratio, const float min_value, const float max_value){
    const float x_phisical_min = inverse_gearbox_transform(SERVO_MIN_ANGLE, SERVO_BIAS, gear_ratio);
    const float x_phisical_max = inverse_gearbox_transform(SERVO_MAX_ANGLE, SERVO_BIAS, gear_ratio);
