...
[keys, params] = get_params_lya_swarm();
...
% Initial states
y0 = zeros([13+4+9 1]);
%Q2 = theta_vector2Q(pi*0.001, [1;0;0]);
%y0(4:7) = Q2;
y0(4:7) = [1; 0; 0; 0]; % Initial orientation
y0(11:13) = [10 10 10];    % Initial position
y0(8:10) = [0 0 0];    % Initial velocity
%y0(14:17) = [0; -0.1248; 0; 0]; % Initial value for servo

% Reference
global ts desire desire2;
syms ts
zeta = [0.1*ts; 1*sin(0.5*ts); 1*cos(0.5*ts); 0*ts];
%zeta = [0; 1*sin(0.1*(ts-1)); ts-15; 0*ts];
%zeta = [0; (0.1*(ts-1)); ts-15; 0*ts];
d_zeta = diff(zeta);
dd_zeta = diff(d_zeta);
...
ddd_zeta = diff(dd_zeta);
dddd_zeta = diff(ddd_zeta);
desire2 = [zeta d_zeta dd_zeta ddd_zeta dddd_zeta];
...
%% Plot
projectpath = 'H:\\.shortcut-targets-by-id\\1_tImZc764OguGZ7irM7kqDx9_f6Tdqwi\\National Taiwan University\\Courses\\110-2\\AdaptiveControl\\FinalProject\\Simulations\\ControllerComparison\\';
foldername = 'test\\';
...
function [dydt, inputs, outputs] = drone_fly(t, y)
    %% Parameters
    [key, params] = get_params_lya_swarm();
