function [dydt, commands, meta] = my_model_so3(params, t, u, y)
    %% Parameters
    g = params('g'); % gravity
...
function [w_m, alpha] = controlAllocation(params, u)
    ...
    % To avoid the emergence of imaginary part
    if Tf < 0
        Tf = 0;
    end

    if Td > prop_d * CP_u / CT_u * Tf
        Td = prop_d * CP_u / CT_u * Tf;
    end
...
function [w_m, alpha] = controlAllocation2(params, u)
    m = params('m'); % Mass, Kg
    rho = params('rho'); % kg/m3
    prop_d = params('prop_d'); % 8 inch = 20.3 cm
