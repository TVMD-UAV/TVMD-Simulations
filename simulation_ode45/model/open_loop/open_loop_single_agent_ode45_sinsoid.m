...
function [dydt, inputs, outputs] = drone_fly(t, y)
    ...
    xi = sin(w0*t+phi) * amp;
    d_xi = w0*cos(w0*t+phi) * amp;
    dd_xi = -w0*w0*sin(w0*t+phi) * amp;
%     xi = 0;
%     d_xi = 0*t;
%     dd_xi = 0*t;
    
    eta = sin(w0*t+phi) * amp;
    d_eta = w0*cos(w0*t+phi) * amp;
    dd_eta = -w0*w0*sin(w0*t+phi) * amp;
%     eta = 0;
%     d_eta = 0*t;
%     dd_eta = 0*t;
    ...
    % Gyroscopic moment
    A_w_P = [0; 0; w_prop_u - w_prop_l];
    B_w_A = [dd_eta; dd_xi; 0];
    ...
    outputs = [I_thrust ; B_M_f; + B_M_d; - B_M_g; - B_M_a; desire; attitude_d'];
end
