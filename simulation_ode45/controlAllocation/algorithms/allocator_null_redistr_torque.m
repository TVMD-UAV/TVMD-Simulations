function [a, b, F, u] = allocator_null_redistr(t_d, conf, a0, b0, f0, W, dt)
    ...
    Mf = M(1:3, :);
    Mt = M(4:6, :);
    [f0, N_esp] = sat(conf, pinv(Mt, pinv_tol) * ut_d);
    ut0 = Mt * f0;
    ...
    while (c < 1) && (rank(Mt_esp) >= 3)
        Mt_esp = Mt * kron(diag(N_esp), eye(3)); % masking
        Mt_esp_dag = pinv(Mt_esp, pinv_tol);
        u_delta = ut_d - ut0;
        f_delta = Mt_esp_dag * u_delta;
    ...
    while (c < 1) && (rank(Mf_esp) >= 3)
        Mf_esp = Mf * kron(diag(N_esp), eye(3)); % masking
        Mt_esp = Mt * kron(diag(N_esp), eye(3)); % masking
        N_Mt_esp = null(Mt_esp);
        z_esp = pinv(Mf_esp * N_Mt_esp, pinv_tol) * uf_delta;
        f_delta = N_Mt_esp * z_esp;
        % ut_delta - Mt * f_delta
