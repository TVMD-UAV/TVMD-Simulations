function [eta, xi, R, F, vecs] = allocator_interior_point(u, W, conf, a0, b0, f0)
    % Configurations
    P = conf('pos');
    psi = conf('psi');
    ...
    Fi = fmincon(@(x)costfun(x, W, u0), x0, [], [], M, u, [], [], @(x)mycon(x, f_max, sigma_a, sigma_b), options);
